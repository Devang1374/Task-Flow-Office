...
new class extends Component
{
    ...
    public function updatedSearch(){
        $this->categorys = auth()->user()->category()->where(
            function($query){
...
<div>
    ...
    <!-- delete category -->
    @if($delete_id)
        <livewire:category.delete :id="$delete_id"/>
    @endif

    <!-- create category -->
    <div wire:key="category-create-container">
        @if($showForm)
            <livewire:category.create wire:key="category-create-component"/>
        @endif
    </div>
                        ...
                        <table class="w-full border-collapse text-left text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">   
                                ...
                                <tr class="hover:bg-gray-50/70 transition-colors dark:hover:bg-gray-700/30">
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$category['title']}}</td>
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$category['caption']}}</td>
                                    <td class="px-6 py-4">
                                      <button 
                                        type="button" wire:click="delete({{$category['id']}})"
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <button wire:click="edit({{$category['id']}})" class="font-medium text-blue-600 hover:underline dark:text-blue-400">Edit</button>
                                    </td>
                                </tr>  
                                @endforeach
                            </tbody>
    ...
    <!-- edit form -->
    <div wire:key="category-update-container">
      @if($isEditing)
        <livewire:category.update :id="$category_id"/>
      @endif
    </div>
</div>