...
new class extends Component
{
    ...
    public function updatedSearch(){
        $this->customers = auth()->user()->customer()->where(
            function($query){
    ...
    public function delete($id){
        $this->delete_id = $id;
        $this->dispatch('sendMessage',msg: $id);
    }
};
?>
...
<div>
    ...
    <!-- delete customer -->
    @if($delete_id)
        <livewire:customer.delete :id="$delete_id"/>
    @endif

    <!-- create customer -->
    <div wire:key="customer-create-container">
        @if($showForm)
            <livewire:customer.create wire:key="customer-create-component"/>
        @endif
    </div>
                        ...
                        <table class="w-full border-collapse text-left text-sm text-gray-500 dark:text-gray-400">
                            <thead class="bg-gray-50 text-xs uppercase tracking-wider text-gray-700 dark:bg-gray-700/50 dark:text-gray-300">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4 font-medium">Number</th>
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">   
                                ...
                                <tr class="hover:bg-gray-50/70 transition-colors dark:hover:bg-gray-700/30">
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$customer['name']}}</td>
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$customer['address']}}</td>
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$customer['number']}}</td>
                                    <td class="px-6 py-4 font-normal text-gray-600 dark:text-gray-300">{{$customer['email']}}</td>
                                    <td class="px-6 py-4">
                                      <button 
                                        type="button" wire:click="delete({{$customer['id']}})"
                                    ...
                                    <td class="px-6 py-4">
                                        <button wire:click="edit({{$customer['id']}})" class="font-medium text-blue-600 hover:underline dark:text-blue-400">Edit</button>
                                    </td>
                                </tr>  
                                @endforeach
                            </tbody>
    ...
    <!-- edit form -->
    <div wire:key="customer-update-container">
      @if($isEditing)
        <livewire:customer.update :id="$customer_id"/>
      @endif
    </div>
</div>